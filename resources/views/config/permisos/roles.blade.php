...
@section('content_header')
    <h1>CONFIGURACION DE ROLES Y PERMISOS</h1>
@stop

@section('content')
    <p>Lista de  <strong>Roles</strong></p>
    <div class="card">
                ...
                @foreach ($roles as $rol)
                    <tr>
                        ...
                        <td>
                            ...
                            <form action=" {{ route('roles.destroy',$rol) }} " method="post" class="formEliminar" style="display: inline">
                                @csrf
                                @method('DELETE')
                                {!! $btnDelete !!}
                            </form>
    ...
    {{-- Themed --}}
    <x-adminlte-modal id="modalNuevoRol" title="Nuevo Rol" theme="primary"
        icon="fas fa-bolt" size='lg' disable-animations>
        <form action="{{route('roles.store')}}" method="post">
            @csrf
            <div class="row">
                <x-adminlte-input name="nombre" label="Nombre" placeholder="Ingresa el nombre del rol"
